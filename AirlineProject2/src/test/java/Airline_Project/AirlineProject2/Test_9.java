...
public class Test_9 extends Launch_Quit
{
	...
	public void login() throws InterruptedException, EncryptedDocumentException, IOException, AWTException
	{
		...
		homepage.multiplePassenger(driver);
		homepage.searchclick(driver);
		
		Amz_flight_SearchResults searchpage=new Amz_flight_SearchResults(driver);
		searchpage.clickbook(driver);
		searchpage.travellerDetails(driver);
		
		Amz_Traveller_Details details=new Amz_Traveller_Details(driver);
		details.bookmultiplepassengers(driver);
